...
export default function InstanceMesh({
  ...
  useFrame((state) => {
    ...
    instanceMeshRef.current.material.uniforms.CamPos.value.copy(
    ...
    let needsUpdate =
      animatedInstances.current.length > 0 ||
      eventDependentInstances.current.length > 0
    if (needsUpdate) {
  ...
  useEffect(() => {
    ...
    if (objectProps.autoGenerateInstance) {
      ...
      for (let i = 0; i < objectProps.instanceCount; i++) {
        ...
        const hasAnimations =
          instances[i].animations && instances[i].animations.length > 0
    ...
    const dynamic =
      ...
      animatedInstances.current.length > 0
    if (dynamic)
      instanceMeshRef.current.instanceMatrix.setUsage(DynamicDrawUsage)
    instanceMeshRef.current.instanceMatrix.needsUpdate = true
    invalidate()
  }, [])
