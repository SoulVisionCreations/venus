...
export const applyEventDrivenAnimations = (
  objectProps,
  time,
  objectRef,
...
export const useEvents = (objectProps, sceneProps, scrolledRotationValue) => {
  const rotateByScrollOnce = (e) => {
    if (
      ...
      scrolledRotationValue.current +=
        animationDefaults.scrollByRotationOnceSpeed *
        (e.wheelDeltaY > 0 ? 1 : -1)
  ...
  const rotateByScrollContinously = (e) => {
    scrolledRotationValue.current +=
      animationDefaults.scrollByRotationContinouslySpeed *
      (e.wheelDeltaY > 0 ? 1 : -1)
  }
  ...
  useEffect(() => {
    if (objectProps.events) {
      objectProps.events.forEach((event) => {
        switch (event.type) {
          case eventDrivenActionTypes.rotateByScrollOnce:
            if (sceneProps.completelyVisibleCount == 0)
              window.addEventListener('wheel', rotateByScrollOnce)
            break
          case eventDrivenActionTypes.rotateByScrollContinously:
