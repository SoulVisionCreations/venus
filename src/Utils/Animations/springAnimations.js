...
import { getTrajectoryPoints } from './curves'
...
export const useSpringAnimation = (instance, sceneProps) => {
  ...
  const [introAnimationCompleted, updateIntroAnimationCompleted] =
    useState(false)
  ...
  useEffect(() => {
    if (chainedAnimations.current.length == 0 || !introAnimationCompleted)
      return
    chainedAnimations.current.forEach((animation) => {
      function executeAnimation(index) {
        ...
        const delay = childAnimation.initialPause
          ? childAnimation.initialPause
          : 0
        const [trajectory, trajectoryVec] = getTrajectoryPoints(
          childAnimation,
          state
        )
        api.start({
          to: async (next) => {
            ...
            while (t < trajectory.length) {
              if (trajectoryVec[t].position)
                state.current.position = trajectoryVec[t].position
              if (trajectoryVec[t].rotation)
                state.current.rotation = trajectoryVec[t].rotation
              if (trajectoryVec[t].scale)
                state.current.scale = trajectoryVec[t].scale
              await next(trajectory[t])
              t++
            }
            if (index + 1 < animation.childAnimations.length)
              executeAnimation(index + 1)
          ...
          delay: delay,
          config: childAnimation.config
            ? childAnimation.config
            : config.default,
        })
        invalidate()
      }
  ...
  useEffect(() => {
    ...
    if (sceneProps.isSceneVisible && !introAnimation.current)
      updateIntroAnimationCompleted(true)
