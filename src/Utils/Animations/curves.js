...
export const getTrajectoryPoints = (animation, state) => {
  ...
  if (animation.trajectory == undefined)
    animation.trajectory = Animation.trajectory.manual
  switch (animation.trajectory) {
    case Animation.trajectory.manual:
      animation.stateIncrements.forEach((increment) => {
        ...
        increment.position &&
          currentState.position.add(arrayToVec(increment.position))
        increment.rotation &&
          currentState.rotation.add(arrayToVec(increment.rotation))
    ...
    case Animation.trajectory.curveDefinedByPoints:
      ...
      const pointsD = getPointsOn3DCurve(
        curve,
        animation.trajectoryMetaData.steps
      )
    ...
    case Animation.trajectory.multipleCurveDefinedByPoints:
      ...
      // )
      const pointsP = getPointsOn3DCurve(
        curve,
        animation.trajectoryMetaData.steps
      )
...
export const getPointsOnEllipse = (data) => {
  ...
  points2D.forEach((point) => {
    ...
    data.rotateTrajectory &&
      data.rotateTrajectory.forEach((rotation) => {
