...
const shouldAnimate = (sceneProps, visibilityThreshold, delta) => {
  ...
  const sceneBottomToScreenBottomDist =
    sceneBottom - (scrolledHeight + screenHeight)
  const sceneBottomToScreenTopDist = sceneBottom - scrolledHeight
  if (sceneProps.id == 2) console.log(sceneHeight)
  if (sceneBottomToScreenTopDist <= 0 || sceneTopToScreenBottomDist <= 0)
    return false
  if (
    delta > 0 &&
    sceneTopToScreenBottomDist / sceneHeight >= visibilityThreshold.current.top
  )
    return true
  if (
    delta < 0 &&
    sceneBottomToScreenBottomDist / sceneHeight >=
      visibilityThreshold.current.bottom
  )
    return true
  return false
}
...
export const useScrollAnimation = (objectProps, sceneProps, springApi) => {
  ...
  useEffect(() => {
    ...
    objectProps.animations.forEach((animation) => {
      if (animation.type == Animation.type.scroll) {
        ...
        if (animation.trajectory)
          scrollTrajectory.current = getTrajectory(animation)
        if (animation.visibilityThreshold != undefined)
          visibilityThreshold.current = animation.visibilityThreshold
        if (
          animation.trajectoryMetaData &&
          animation.trajectoryMetaData.speed != undefined
        )
  ...
  const updateStateByDelta = (deltaY) => {
    if (scrollTrajectory.current) {
      ...
      state.current.position = scrollTrajectory.current.getPoint(
        trajectoryState.current
      )

      if (scrollAnimation.current.trajectoryMetaData.rotationTrajectory) {
        const rotationTrajectory =
          scrollAnimation.current.trajectoryMetaData.rotationTrajectory
      ...
      if (scrollAnimation.current.trajectoryMetaData.scaleTrajectory) {
        const scaleTrajectory =
          scrollAnimation.current.trajectoryMetaData.scaleTrajectory
    ...
    if (scrollAnimation.current.rotationMetaData) {
      ...
      state.current.rotation.add(
        new Vector3(
          axis[0] * velocity * deltaY,
          axis[1] * velocity * deltaY,
          axis[2] * velocity * deltaY
        )
      )
    }
    if (scrollAnimation.current.scaleMetaData) {
      ...
      state.current.scale.add(
        new Vector3(
          scaleRatio[0] * velocity * deltaY,
          scaleRatio[1] * velocity * deltaY,
          scaleRatio[2] * velocity * deltaY
        )
      )
  ...
  useEffect(() => {
    if (sceneProps.isSceneVisible && scrollAnimation.current) {
      if (window.pageYOffset > 0) {
        ...
        const scrolledHeight = window.pageYOffset
        const screenHeight = window.innerHeight
        const sceneTopToScreenBottomDist =
          scrolledHeight + screenHeight - sceneTop
        const delta =
          sceneTopToScreenBottomDist -
          visibilityThreshold.current.top * sceneHeight
        if (delta > 0) {
