...
export const animateMesh = (time, meshConfig, meshRef) => {
  const [finalPosition, finalRotation, finalScale] = getAnimatedTransformation(time, meshConfig);
...
export const getAnimatedTransformation = (time, instanceConfig) => {
  let [positionChange, rotationChange, scaleChange] = [new Vector3(0,0,0), new Vector3(0,0,0), new Vector3(0,0,0)];

  instanceConfig.animations.forEach(animation => {
    let [positionDelta, rotationDelta, scaleDelta] = [new Vector3(0,0,0), new Vector3(0,0,0), new Vector3(0,0,0)];
    switch(animation.type) {
        ...
        case AnimationTypes.Float :
            [positionDelta, rotationDelta, scaleDelta] = floatAnimation(time, animation);
            break;
    }
...
export const floatAnimation = (time, animation) => {
...
export const rotateAnimation = (time, animation) => {
  ...
  const rotation = new Vector3(0,0,0);
  const scale = new Vector3(0,0,0);
  const rotationArray = animation.rotationArray ? arrayToVec(animation.rotationArray) : arrayToVec(animationDefaults.rotationArray);
